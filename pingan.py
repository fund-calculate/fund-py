...
class Fund:
    ...
    def __init__(self, code):
        ...
        # 判断产品是否存在
        if 'responseMsg' in resp:
            self.name = resp['responseMsg']
        ...
        for stock in resp['stocks']:
            self.stocks.append(
                Stock(stock['stockPrtStockCode'], stock['stockPrtStockName'], stock['stockPrtstkvaluetonav']))
    ...
    def print_base(self):
        print('| ', self.code.ljust(8), self.name,
              self.unitTotal.rjust(11 - zh_count(self.unitTotal)), self.setupDate.rjust(12), ' |')
        # print('| ', self.code.ljust(8), self.fullName.ljust(60 - zh_count(self.fullName)),
        #       self.unitTotal.rjust(11 - zh_count(self.unitTotal)), self.setupDate.rjust(12), ' |')
